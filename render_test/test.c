...
int	init_graphics(t_mlx *mlx, t_parse_data *data)
{
	...
	if (!init_textures(data, mlx))
		...
		mlx_destroy_display(mlx->mlx), free(mlx->mlx), 1); //needs free
	if (!create_2d_player(&player, mlx, data))
		return (print_error("Error\ntextures", 'n'),
		mlx_destroy_image(mlx->mlx, minimap.img_2d), cleanup_parse(data, mlx), mlx_destroy_window(mlx->mlx, mlx->mlx_win), mlx_destroy_display(mlx->mlx), free(mlx->mlx), 1); //needs free
